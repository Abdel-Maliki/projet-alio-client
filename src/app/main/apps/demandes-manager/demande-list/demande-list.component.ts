...
export class ListDemandesComponent implements OnInit, OnDestroy {
    ...
    commentaireSignateur(demande: Demande): void {
        this.dialogRef = this._matDialog.open(FormCommentaireComponent, {
            panelClass: 'commentaire-form-dialog',
            data: {
                demande: demande,
                action: 'edit'
            }
        ...
        this.dialogRef.afterClosed()
            .subscribe(response => {
                ...
                this._demandesService.signatureDemande(demande);
                // this._demandesService.updateDemande({requestId: demande ? demande.id : 0, isApproved: actionType}).then();
